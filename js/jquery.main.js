...
function initCustomHover() {
	jQuery('.nav-menu, .cards-list').touchHover({});
}

function initHeader() {
	var win = jQuery(window);
	var header = jQuery('#header');
	var customClass = 'alt-bg';
	...
	win.on({
		load: checkScroll,
		scroll: checkScroll,
		orientationchange: checkScroll,
		resize: checkScroll
	});
}

 function initFixedBlock() {
   var item = jQuery('.fixed-btn');
   var win = jQuery(window);
   var winH = win.height();
   var itemOffSet = jQuery('#footer');
   var top = itemOffSet.offset().top;
   fixedItem();
